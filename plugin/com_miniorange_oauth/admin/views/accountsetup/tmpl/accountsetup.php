...
function selectAppByIcon()
{
    ...
    $listHtml = "<ul id='moAuthAppsList'>";
    $PreConfiguredApps = array_slice($appArray, 0, count($appArray) - 2);
    foreach ($PreConfiguredApps as $key => $value) {
        $listHtml .= "<li class='mo_boot_border' moAuthAppSelector='" . $value['label'] . "'>";
        $listHtml .= "<a class='mo_boot_select_app' href='" . JRoute::_('index.php?option=com_miniorange_oauth&view=accountsetup&moAuthAddApp=' . $key) . "'>";
