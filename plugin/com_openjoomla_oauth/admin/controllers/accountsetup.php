...
class openjoomlaoauthControllerAccountSetup extends JControllerForm
{
    ...
    public function saveConfig()
    {
        ...
        if (count($post)==0) {
            ...
            if (isset($post['callbackurl'])) {
                ...
                $appname                   = isset($post['oj_oauth_app_name'])? $post['oj_oauth_app_name'] : '';
                $db     = JFactory::getDbo();
                ...
                $returnURL  = 'index.php?option=com_openjoomla_oauth&view=accountsetup&ojAuthAddApp='.$post['oj_oauth_app_name'];
            ...
            $clientid                = isset($post['oj_oauth_client_id'])? $post['oj_oauth_client_id'] : '';
            $clientsecret            = isset($post['oj_oauth_client_secret'])? $post['oj_oauth_client_secret'] : '';
            $scope                   = isset($post['oj_oauth_scope'])? $post['oj_oauth_scope'] : '';
            $appname                 = isset($post['oj_oauth_app_name'])? $post['oj_oauth_app_name'] : '';
            $customappname           = isset($post['oj_oauth_custom_app_name'])? $post['oj_oauth_custom_app_name'] : '';
            $appEndpoints            = json_decode($appD->getAppJson(), true);
            $appEndpoints            = $appEndpoints[$appname];
            $authorizeurl            = isset($post['oj_oauth_authorizeurl'])? $post['oj_oauth_authorizeurl'] : '';
            $accesstokenurl          = isset($post['oj_oauth_accesstokenurl'])? $post['oj_oauth_accesstokenurl'] : '';
            $resourceownerdetailsurl = isset($post['oj_oauth_resourceownerdetailsurl'])? $post['oj_oauth_resourceownerdetailsurl'] : '';
            $current = "";
            ...
            $in_header               = isset($post['oj_oauth_in_header'])?$post['oj_oauth_in_header']:'';
            $enableOAuthLoginButton  = isset($post['login_link_check']) ? $post['login_link_check'] : '0';
            $in_body                 = isset($post['oj_oauth_body'])?$post['oj_oauth_body']:'';
    ...
    public function saveMapping()
    {
        ...
        $email_attr = isset($post['oj_oauth_email_attr'])? $post['oj_oauth_email_attr'] : '';
        $full_name_attr = isset($post['oj_oauth_full_name_attr'])? $post['oj_oauth_full_name_attr'] : '';
        $user_name_attr = isset($post['oj_oauth_user_name_attr'])? $post['oj_oauth_user_name_attr'] : '';
        ...
        $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup&tab-panel=configuration&progress=step4', JText::_('COM_OPENJOOMLA_OAUTH_ATTRIBUTE_MAPPING_SAVED_SUCCESSFULLY'));
    }
    ...
    public function clearConfig()
    {
        ...
        $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup&tab-panel=configuration', JText::_('COM_OPENJOOMLA_OAUTH_APP_CONFIGURATION_RESET'));
    }
    ...
    public function exportConfiguration()
    {
        ...
        if ($clientid =='' && $clientsecret =='') {
            $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup', JText::_('COM_OPENJOOMLA_OAUTH_ENTER_CLIENT_ID_BEFORE_DOWNLOADING'), 'error');
            return;
        }
        ...
        $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup', JText::_('COM_OPENJOOMLA_OAUTH_PLUGIN_CONFIGURATION_DOWNLOADED_SUCCESSFULLY'));
        exit;
    }
    ...
    public function moOAuthProxyConfigReset()
    {
        ...
        $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup&tab-panel=account', JText::_('COM_OPENJOOMLA_OAUTH_PROXY_SETTING_RESET'));
    }
    ...
    public function moOAuthProxyServer()
    {
        ...
        $this->updateDatabaseQuery($nameOfDatabase, $updateFieldsArray);
        $this->setRedirect('index.php?option=com_openjoomla_oauth&view=accountsetup&tab-panel=account', JText::_('COM_OPENJOOMLA_OAUTH_PROXY_SERVER_SAVED_SUCCESSFULLY'));
    }
}
