...
class OpenOAuthHandler
{
    ...
    public function getAccessToken($tokenendpoint, $grant_type, $clientid, $clientsecret, $code, $redirect_url, $in_header_or_body)
    {
        ...
        if (curl_error($ch)) {
            ...
            $session->set('oj_reason', curl_error($ch));
        }

        $content =json_decode($content, true);
        if (!is_array($content)) {
            $this->setError("Invalid response received.");
            $session->set('oj_reason', 'Invalid response received.');
        }

        // first check if any error received
        if (isset($content["error_description"])) {
            $this->setError($content["error_description"]);
            $session->set('oj_reason', $content["error_description"]);

        } elseif (isset($content["error"])) {
            $this->setError($content["error"]);
            $session->set('oj_reason', $content["error"]);
        }
        ...
        if (empty($idToken) && empty($access_token)) {
            ...
            $session->set('oj_reason', 'Invalid response received from OAuth Provider. Contact your administrator for more details.');
        }
    ...
    public function getResourceOwnerFromIdToken($id_token)
    {
        ...
        $session->set('oj_reason', 'Invalid response received.<br><b>Id_token : </b>'.$id_token);
        return false;
    }
